...
It adds the project root folder to sys.path and then starts the manager.
The DataSourceManager publishes its registration on the "manager_registry" channel
periodically (every 5 seconds), so that any client (e.g. a PyQt frontend) will eventually
...
# Add the project root folder to sys.path (assuming this script is in the project root)
