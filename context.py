...
COMMON_AVOID_FOLDERS = BBConfig.get('context_avoid_folders')

# Common files to avoid
COMMON_AVOID_FILES = BBConfig.get('context_avoid_files')


class ProjectContextGenerator:
    def __init__(self, config):
        self.root_dir = config['root_dir']
        self.avoid_folders = COMMON_AVOID_FOLDERS
        self.avoid_files = set(COMMON_AVOID_FILES)
    ...
    def generate_context_file(self):
        BBLogger.log(f"Generating context file: {self.output_file}")
        project_data = {
                ...
                for file in files:
                    ...
                    if (
                        ...
                        try:
                            ...
                            BBLogger.log(f"Skipping file {file_path} due to decoding error: {e}")
                        except Exception as e:
                            BBLogger.log(f"Skipping file {file_path} due to an unexpected error: {e}")
        ...
        BBLogger.log(f"Context file generated at: {self.output_file}")

    def split_file(self, file_path, num_chunks=None, chunk_size=None):
        output_dir = f"{os.path.splitext(file_path)[0]}_parts"
        os.makedirs(output_dir, exist_ok=True)
        BBLogger.log(f"Splitting file {file_path} into parts in directory {output_dir}")
        ...
        for i in range(0, len(content), chunk_size):
            ...
            BBLogger.log(f"Created part file: {part_filename}")
            part_num += 1
        ...
        BBLogger.log(f"Moved original context file to: {new_output_file_path}")
...
def main(
    ...
    if compress:
        ...
        if parts_dir_abs != new_parts_dir_abs:
            ...
            BBLogger.log(f"Parts directory moved to: {new_parts_dir}")
        else:
            BBLogger.log(f"Parts directory is already in the output folder: {new_parts_dir}")
